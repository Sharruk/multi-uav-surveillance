...
from simulation.config import (
    ...
    GPS_NOISE, COMM_DELAY, NUM_PEOPLE,
...
def draw_panel(surf, fh, fsm, fxs,
               drones: list, crowd,
               metrics: dict, elapsed: float, paused: bool) -> None:
    ...
    Args:
        ...
        fh:      header font
        fsm:     small font
        fxs:     extra-small monospace font
        drones:  list of Drone instances
        crowd:   CrowdSystem instance
        metrics: dict from analytics.calc_metrics()
    ...
    # ── Inner helpers ──────────────────────────────────────────────────────────
    def row(lbl: str, val, lc=C_TEXT_SEC, vc=C_TEXT_PRI) -> None:
        nonlocal y
    ...
    row("Tracking Accuracy",
        f"{metrics['accuracy']:.1f}%",
        vc=C_TEXT_OK if metrics["accuracy"] > 70 else C_TEXT_WARN)
    ...
    for d in drones:
        ...
        sv = fxs.render(f"Spd: {spd_px:.2f} px/f | Dist: {d.distance * 0.1:.0f}m",
                        True, C_TEXT_SEC)
